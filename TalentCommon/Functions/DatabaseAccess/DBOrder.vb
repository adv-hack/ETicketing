...
'
'       Error Number Code base      TACDBOR- 
'                                   
'       Modification Summary
'
'       dd/mm/yy    ID      By      Description
'       --------    -----   ---     -----------
'       19/02/07    /001    Ben     Change length of alt sku to 36
'--------------------------------------------------------------------------------------------------
<Serializable()> _
Public Class DBOrder
    Inherits DBAccess

#Region "miscItems Inner Class"
    '----------------------------------------------------------------------------------------------
    ...
    Private Class miscItems
        ...
        Public Sub New(ByVal pPaymentType As String, _
                        ...
                        ByVal pOrderFailedInitialValidationMessage As String, _
                        ByVal pPromotion_Codes As String())
            ...
            PromotionCodes = pPromotion_Codes
        End Sub
...
#Region "Private Methods"
    Private Function WriteDataBaseChorus() As ErrorObj
        ...
        Return err
    End Function
    Private Function WriteDataBaseTalentTKT() As ErrorObj
        '---------------------------------------------------------------------------
        '   AccessDataBaseSystem21: Write the order to Chorus via Stored Procedures
        '
        Dim err As New ErrorObj
            ...
            Try
                ...
                For iOrder = 1 To 500
                    ...
                    If ParmHEAD(iOrder) > String.Empty Then
                        ...
                        '   Call order header stored procedure towrite to Talent (ORDERREQ)
                        '
                        Dim contimeOut As Integer = conTALENTTKT.ConnectionTimeout
                        cmdSelectORDERREQ = New iDB2Command(strSqlORDERREQ, conTALENTTKT)
                        ...
                        Else
                            ...
                            For iItems = 1 To 1000
                                ...
                                If ParmITEM(iOrder, iItems) > String.Empty Then
                                    ...
                                    '   Call order detail stored procedure to write to Talent (ORDERREQ2)
                                    ' 
                                    cmdSelectORDERREQ2 = New iDB2Command(strSqlORDERREQ2, conTALENTTKT)
                                    Dim strItemLine As String = ParmITEM(iOrder, iItems)
                                    PARMOUT = String.Empty
                                    parmInput = cmdSelectORDERREQ2.Parameters.Add(Param1, iDB2DbType.iDB2Char, 1024)
                                    parmInput.Value = strTalentOrderNo & strItemLine.Substring(15)
                            ...
                            For iComments = 1 To 500
                                If ParmCOMM(iOrder, iComments) > String.Empty Then
                                    ...
                                    cmdSelectWRITETXT = New iDB2Command(strSqlWRITETXT, conTALENTTKT)
                                    PARMOUT = String.Empty
                                    parmInput = cmdSelectWRITETXT.Parameters.Add(Param1, iDB2DbType.iDB2Char, 1024)
                                    If ParmCOMMLineNumber(iOrder, iComments) <> "" Then
                                        parmInput.Value = strTalentOrderNo & ParmCOMMLineNumber(iOrder, iComments).PadLeft(6, "0")
                            ...
                            If err.ItemErrorCode(iOrder) = String.Empty Then
                                ...
                                '   Last item now written - if no error then write order to Chorus (WRITEORD)
                                ' 
                                cmdSelectWRITEORD = New iDB2Command(strSqlWRITEORD, conTALENTTKT)
                                Dim strWriteOrd As String = String.Empty
                                Dim strTotalOrderValue As String = (ParmMISC(iOrder).TotalOrderValue * 100).ToString("0000000000000000").PadLeft(16, "0")
                                ...
                                Else
                                    ...
                                    ' Order placed OK - decrement stock..
                                    Dim deos As New DeOrders
                                    Dim deoi As New DEOrderInfo
                                    Dim rowsAffected As Integer = 0
                                    Dim depr As New DeProductLines
                                    Dim _TDataObjects As Talent.Common.TalentDataObjects = New Talent.Common.TalentDataObjects()
                                    ...
                                    _TDataObjects.Settings.DestinationDatabase = "SQL2005"
                                    ...
                                    For iItems = 1 To deoi.CollDEProductLines.Count
                                        ...
                                        With depr
                                            ...
                                            '     Dim productInfo As New Talent.Common.DataObjects.Products
                                            rowsAffected = _TDataObjects.ProductsSettings.TblProductStock.DecrementStockByProductCode(.SKU, .Quantity, False)

                                            '   _TDataObjects.Settings
        ...
        End If
        ' order gets created twice otherwise..
        Settings.DestinationDatabase = "TALENTTKT"
        Return err
    End Function
                            ...
                            Else
                                ...
                                If err.ItemErrorCode(iOrder) = String.Empty Then
                                    ...
                                    parmInput.Value = Utilities.FixStringLength(Settings.AccountNo1, 8) & "  " & strTalentOrderNo & _
                                                                ...
                                                                ParmMISC(iOrder).TotalValueCharged.ToString("0000000000000.00") & _
                                                                Utilities.FixStringLength(Settings.AccountNo3, 2) & _
                                                                   ParmMISC(iOrder).TotalOrderValue.ToString("0000000000000.00") & _
                                                                   GetPromoCodesInFixedLength(ParmMISC(iOrder).PromotionCodes)
    ...
    Private Function WriteDataBaseSQL2005() As ErrorObj
        ...
        Try
            'Currently the order does not need to be moved to anywhere (eg. a third party database).
            'The order status needs updating.
            Dim tDataObjects As New TalentDataObjects
            Dim webOrderId As String = String.Empty
            Dim SmartcardNumber As String = String.Empty
            Dim deOrders As DeOrders = Dep.CollDEOrders(1)
            webOrderId = deOrders.DEOrderHeader.WebOrderNumber
            SmartcardNumber = deOrders.DEOrderHeader.SmartcardNumber
            If webOrderId.Length > 0 Then
                tDataObjects.Settings = Settings
                tDataObjects.OrderSettings.TblOrderHeader.UpdateOrderStatusandSmartcard(Settings.BusinessUnit, webOrderId, Utilities.GetOrderStatus("ORDER PENDING"), Settings.LoginId, SmartcardNumber)

            Else
                With err
                    .ErrorMessage = strError6
            ...
            With Dep
                ...
                For iOrder = 1 To .CollDEOrders.Count
                    ...
                    ParmMISC(iOrder) = New miscItems(Utilities.FixStringLength(deoh.PaymentType, 20), _
                                                     ...
                                                     orderFailedInitialValidationMessage, _
                                                     deoh.PromotionCodes)
            ...
            With Dep
                ...
                For iOrder = 1 To .CollDEOrders.Count
                    ...
                    If Not Settings.AccountNo2 Is Nothing AndAlso Settings.AccountNo2.Length > 3 Then
                        Settings.AccountNo2 = Settings.AccountNo2.Substring(0, 3)
                    End If
                    ' dont use billing address for talentTKT
                    If Settings.DestinationDatabase = TALENTTKT Then
                        ParmHEAD(iOrder) = BuildParmHEAD_CHORUS(Settings.AccountNo1, Settings.AccountNo2, deoh, "", deoh.CollDECharges, deoh.CollDEAddress(1), Settings.AccountNo3, Settings.LoginId)
                    Else
    ...
    Private Function BuildParmHEAD_CHORUS(ByVal customerNo As String, ByVal deliverySeq As String, ByVal orderHeader As DeOrderHeader, _
        ...
        If Settings.DestinationDatabase = TALENTTKT AndAlso parmWebOrderNo.Substring(0, 13) = "UNITEDKINGDOM" Then
            ' Strip BU from order no as it is too long for xxcop100
            parmWebOrderNo = Utilities.FixStringLength(orderHeader.WebOrderNumber.Substring(13), 15)
    ...
    Private Function BuildParmITEM_CHORUS(ByVal orderNo As String, ByVal depr As DeProductLines, ByVal customerNo As String, _
        ...
        Dim parmProductDescription As String = Utilities.FixStringLength(depr.ProductDescription, 50)
        ...
        With sb
            ...
            .Append(parmShipAddPost).Append(parmShipToAtt).Append(parmLineNo).Append(parmPrice)
            .Append(parmShipCountry).Append(parmShipEmail).Append(parmPriceNet).Append(parmPriceTax)
            .Append(parmLineComment).Append(parmCompanyNo).Append(promoPercentage).Append(parmProductDescription)
        End With
    ...
    ''' <summary>
    ''' Gets all the promotion codes in the fixed string length from string array
    ''' </summary>
    ''' <param name="promoCodesArray">The promo codes array.</param>
    ''' <returns>string of </returns>
    Private Function GetPromoCodesInFixedLength(ByVal promoCodesArray As String()) As String
        Dim promotionCodes As String = String.Empty
        If promoCodesArray.Length > 0 Then
            For codeIndex As Integer = 0 To promoCodesArray.Length - 1
                promotionCodes = promotionCodes & Utilities.FixStringLength(promoCodesArray(codeIndex), 20)
            Next
        End If
        promotionCodes = Utilities.FixStringLength(promotionCodes, 100)
        Return promotionCodes
    End Function
#End Region

#Region "Read Order Detail Commented"

...
namespace TalentSystemDefaults
{
	...
	{
		public class tbl_page_attribute : DBObjectBase
		{
			...
			const string TBL_PAGE_ATTRIBUTE = "tbl_page_attribute";
			#endregion
			#region Constructors
			/// <summary>
			/// Constructor
			...
			/// <remarks></remarks>
			public tbl_page_attribute(ref DESettings settings)
				: base(settings)
			{
			...
			public override void RetrieveCurrentValues(List<ConfigurationItem> configs)
			{
			...
			public override List<ConfigurationEntity> RetrieveAllValues(string businessUnit, string[] defaultKeys, string[] variableKeys, string displayName = "")
			{
				...
				string commandText = "SELECT * FROM tbl_page_attribute WHERE business_unit = @BUSINESS_UNIT AND page_code = @PAGE_CODE" + (displayName != string.Empty ? " AND ATTR_NAME like \'%\' + @ATTR_NAME + \'%\'" : "");
			...
			private Dictionary<string, string> RetrieveCurrentValues(string businessUnit, System.Collections.Generic.List<ConfigurationItem> configs)
			{
				...
				try
				{
					...
					{
						whereClause[i] = "(attr_name = @ATTR_NAME" + i.ToString() + " AND page_code = @PAGE_CODE" + i.ToString() + ")";
					}
					where = string.Join(" OR ", whereClause);
					commandText = string.Format("SELECT * FROM tbl_page_attribute WHERE business_unit = @BUSINESS_UNIT AND ({0})", where);
					//Construct The Call
					talentSqlAccessDetail.Settings = settings;
					talentSqlAccessDetail.CommandElements.CommandExecutionType = CommandExecution.ExecuteDataSet;
					talentSqlAccessDetail.CommandElements.CommandParameter.Clear();
			...
			private List<ConfigurationEntity> GetConfigurationData(DataTable dt, string[] defaultKeys, string[] variableKeys)
			{
				...
				foreach (DataRow row in dt.Rows)
				{
					...
					results.Add(new ConfigurationEntity(TBL_PAGE_ATTRIBUTE, string.Empty, defaultKeys, variableKeys, dispName, attrName, attrValue, string.Empty, string.Empty, attrDescription));
				}
				return results;
			}
			/// <summary>
			...
			private int UpdateData(string businessUnit, System.Collections.Generic.List<ConfigurationItem> configs, SqlTransaction givenTransaction)
			{
				...
				for (int i = 0; i <= configs.Count - 1; i++)
				{
					cases.Append(" WHEN  @ATTR_NAME" + i.ToString() + " THEN  @ATTR_VALUE" + i.ToString());
				}
				string[] whereClause = new string[configs.Count - 1 + 1];
				for (int i = 0; i <= configs.Count - 1; i++)
				{
				...
				var commandText = string.Format("UPDATE tbl_page_attribute SET attr_value = CASE attr_name {0} END  WHERE business_unit = @BUSINESS_UNIT AND ({1})", cases.ToString(), where);
				int affectedRows = 0;
				TalentDataAccess talentSqlAccessDetail = new TalentDataAccess();
				ErrorObj err = new ErrorObj();
				//Construct The Call
				...
				{
					talentSqlAccessDetail.CommandElements.CommandParameter.Add(ConstructParameter("@ATTR_NAME" + i.ToString(), configs[i].DefaultName));
					talentSqlAccessDetail.CommandElements.CommandParameter.Add(ConstructParameter("@ATTR_VALUE" + i.ToString(), configs[i].UpdatedValue));
					talentSqlAccessDetail.CommandElements.CommandParameter.Add(ConstructParameter("@PAGE_CODE" + i.ToString(), configs[i].DefaultKey1));
				}
				//Execute
				...
				try
				{
					...
					if (missingConfigs.Count > 0)
					{
						...
						for (int i = 0; i < missingConfigs.Count; i++)
						{
							...
							String text = string.Format("SELECT @BUSINESS_UNIT, @PARTNER_CODE, @PAGE_CODE_{0}, @ATTR_NAME_{0}, @ATTR_VALUE_{0}, @DESCRIPTION_{0}", j);
						...
						commandText = string.Format("INSERT into tbl_page_attribute {0}", values);
						talentSqlAccessDetail.CommandElements.CommandText = commandText;

...
namespace TalentSystemDefaults
{
	...
	{
		public class CD00A : DBObjectBase
		{
			#region Class Level Fields
			private const string CD00A_Renamed = "CD00A";
			#endregion
			...
			public CD00A(ref DESettings settings)
				: base(settings)
			{
			...
			public override void RetrieveCurrentValues(System.Collections.Generic.List<ConfigurationItem> configs)
			{
				...
				foreach (ConfigurationItem config in configs)
				{
					...
					{
						currentValue = System.Convert.ToString(currentValues[config.DefaultName]);
			...
			private List<ConfigurationEntity> GetConfigurationData(DataTable dt, string[] defaultKeys, string[] variableKeys)
			{
				...
				foreach (DataRow row in dt.Rows)
				{
					foreach (System.Data.DataColumn column in dt.Columns)
					{
						...
						results.Add(new ConfigurationEntity(CD00A_Renamed, string.Empty, defaultKeys, variableKeys, dispName, defaultName, defaultValue, string.Empty, string.Empty));
					}
				}
				return results;
			}
			...
			/// <returns></returns>
			/// <remarks></remarks>
			private Dictionary<string, string> GetCurrentValues(DataTable dt, List<ConfigurationItem> configs)
			{

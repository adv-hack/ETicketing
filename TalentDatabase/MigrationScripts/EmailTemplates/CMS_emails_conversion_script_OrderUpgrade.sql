...
if @count > 0 
	...
	begin
		...
		set @subject = isnull((select top 1 CONTROL_CONTENT from tbl_control_text_lang where CONTROL_CODE = 'OrderConfirmEmail.vb' and TEXT_CODE = 'EmailSubjectTicketingUpgrade'),'')
		set @body =     ((convert(nvarchar(max),isnull((select top 1 CONTROL_CONTENT from tbl_control_text_lang where CONTROL_CODE = 'TicketingUpgrade.vb' and TEXT_CODE = 'EmailMergeCode01'),''))) + ' ' +
...
set @html = (select top 1value from tbl_ecommerce_module_defaults_bu where DEFAULT_NAME = 'SEND_ORDER_CONFIRMATION_EMAIL_AS_HTML')
